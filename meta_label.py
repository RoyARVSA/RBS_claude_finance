...
def purged_walk_forward(samples: list[dict], n_folds: int = N_FOLDS, embargo: int = EMBARGO_DAYS,
                        l2: float = 1.0, size_cfg: dict | None = None) -> dict:
    """
    ...
    out = {"n_oos": int(len(P)), "auc": auc_score(Y, P), "logloss": _logloss(Y, P),
           ...
           "skip_rate": float((sizes == 0).mean()), "folds": folds, "calib": calib}
    ok, why = gate(out)
...
def train(samples: list[dict], as_of: str, l2: float = 1.0) -> dict:
    """purged walk-forward 驗證 → 全樣本重訓 → meta.json 內容。"""
    base = (float(np.mean([s["win"] for s in samples])) if samples else None)
    size_cfg = size_cfg_from_base(base)
    oos = purged_walk_forward(samples, l2=l2)                    # 各折尺寸門檻用訓練段基準；最終模型存全樣本基準
    model = {"version": 1, "as_of": as_of, "features": list(FEATURES), "n": len(samples),
             "base_rate": base,
    ...
    if samples and len({s["win"] for s in samples}) == 2:
        ...
        model.update(fit_logistic(X, y, l2))
        mech = {}
        for s in samples:
...
def load_model(path: Path | None = None) -> dict | None:
    try:
        ...
        return d if isinstance(d, dict) and d.get("coef") else None
    except Exception:
        return None
...
def usable(model: dict | None) -> bool:
    return bool(model and model.get("coef") and model.get("gate_passed"))
...
def meta_text(model: dict | None) -> str:
    ...
    o = model.get("oos") or {}
    lines = [f"🧪 *交易層 meta-labeling*（{model.get('as_of')}｜樣本 {model.get('n')}｜基準勝率 {_f(model.get('base_rate'))}）",
...
if __name__ == "__main__":
    ...
    model = train(samples, "2026-09-19")
    o = model["oos"]
    ...
    m_sh = train(shuffled, "2026-09-19")
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        back = load_model(p)
        assert usable(back) and back["features"] == list(FEATURES)
