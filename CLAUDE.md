...
| Alpaca 模擬交易 | `trade_engine.py`（Lean 式分層引擎 `decide`：訊號只管進場、出場走停損/追蹤/分批/死錢、保險絲三態；進場品質層：追高濾網 `ext_atr`／中性 regime 縮量／加碼閘，PITFALLS D15；state["engine"] 簿記；`reflection.throttle` 反思節流在 scan_signals 接）+ `alpha_overlay.py`（Alpha 層資訊疊加：內部人/選擇權/空單/財報 veto/恐貪縮倉，state["alpha_cache"] 限額輪替）+ `alpaca_trader.py`（REST client + bracket 單；`decide_orders` 為 legacy 退回路徑）|
