...
| 候選篩選 / 治理月報（P5/P6） | `screener.py`（/screen：候選池 = universe.top ∪ 主題 − watchlist；Stage 3 ≤8 檔/次、state["screen"] 明文快取；`maybe_refresh_screen` 閉市輪每週）+ `val_report.py`（/valreport：覆蓋/事後命中/穩定度/MoS IC/指引；`_should_send_valreport` 每月一次）|
