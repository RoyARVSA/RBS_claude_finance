...
## 📌 覆盤發現（2026-08-18 首次期中評價）

- **pyramid 死鎖已修**（headroom 1.3 + 部分加碼）：production 驗屍證實
  ATR sizing（1% risk ÷ 5-8% 停損）天然把進場開到 15% 滿格，舊「市值+加碼
  ≤15%」條件數學上恆 False——上線一個月 8/8 檔零觸發。教訓：**參數要放在
  同一張紙上算幾何**，單獨合理 ≠ 合起來可行
- 追蹤中：Shadow 引擎增量 -1.41pp（8/18；單邊漲+急回劇本對新引擎最不利，
  影子帳滿倉尚未付過下跌代價——8/29 滿月覆盤再判）；訊號近 20 筆命中率
  35%（<45% 持續則問題在 alpha 端，動評分校準非引擎）

## 🧭 第三層：方向性 roadmap（實驗驅動，讓數據決定）

> 📐 **估值層（公司財務前瞻 → 資金佈建）的完整規劃見 [VALUATION_PLAN.md](VALUATION_PLAN.md)**（2026-09-07，源自使用者 VRT 三表模型的解剖；分 P0–P5、預設關閉、holdout 通過才接進引擎）；開源生態對照、差異化十項與選股池 v2 見 [VALUATION_LANDSCAPE.md](VALUATION_LANDSCAPE.md)（2026-09-08）。 **進度看板見 VALUATION_PLAN §10（2026-09-09：P0–P4 落地，P3 預設關閉待 A/B）。**
