...
### B10. SEC EDGAR 對雲端機房整段封鎖（五層洋蔥實案，2026-08）
- **症狀**：GitHub Actions 上內部人資料全滅，且每層修完都「還是查無」。
  實際上是五個獨立問題疊在一起：(1) secret 設錯位置（Actions 之外）→
  (2) secret 值尾端換行炸 header（D5 續集）→ (3) SEC 不回 403 而是掛連線等
  逾時，一輪 22 請求×15s 把 cron 從 15 分拖成每小時 → (4) 自家熔斷器誤殺
  CIK 備援地圖（省時機制殺了換路機制）→ (5) 最底層真相：SEC 對 GitHub
  機房 IP 整段封鎖，UA 合規也沒用。
- **修法/教訓**：診斷靠「每一層都留狀態碼日誌」逐層剝（黑箱時先補可觀測性
  再猜原因）；最終解=換資料源（Finnhub insider API，`fetch_insider_any`
  雙源，SEC 優先自動退援）。**雲端跑的東西，別假設任何公開資料源永遠直連
  可達——每個源都要有備援或可缺席。**
- **副產物**：熔斷器（首次逾時即放棄本輪 SEC）防止外部服務拖垮 cron 節奏，
  此模式可複製到其他慢源。
...
### D15. 強訊號進場後 5 日平均是負的——問題不在方向、在「買在延伸端」

- **症狀**（2026-09-14 診斷，反思帳本 60 筆 + journal 71 筆）：評分 ≥0.5 看多判斷 5 日命中率 33%、
  平均 −1.1%；|score|≥0.65 的組（30%）不比 0.5–0.65（33%）好；硬停損 6/14 在進場 1–2 天內被打到
  （VRT 兩次隔日 −8～−9%、DRAM 隔日 −9.7%）；金字塔加碼已平 0/2 勝（−7%）。同時期分批鎖利 5/5 勝 +8.7%。
- **原因**：綜合評分 = 趨勢 35% + MACD 25% + 1 月動量 20%，分數最高的那天幾乎就是延伸最大的那天；
  1.5×ATR 的硬停損在噴出後的正常回檔內。加碼在 +1R 觸發，等於在延伸端再買一次。
  訊號「方向」沒錯（分批鎖利的都是同一批訊號），錯的是進場時點與加碼時點。
- **修法**：不抬門檻（抬門檻只會挑到更延伸的標的）。scan 帶 `ext_atr=(價−MA20)/ATR`；引擎
  `entry_max_ext_atr`（預設 2）且 5 日漲幅 > `entry_max_ret5d`（6%）才算噴出、不開新倉「等回檔」（等速平穩趨勢的 ext 是常數，只看 ext 會永遠不進場）、`pyramid_max_ext_atr`（1.5）、`pyramid_min_score` 0.5；
  regime=neutral 新倉風險 ×0.5 且不加碼；氣象台廣度 <40 把 risk_on 否決成 neutral（實案：總分 66、廣度 36）；
  反思節流：近 30 次看多命中 <40% → 風險再 ×0.5、停加碼；FOMC 會期兩天不開新倉（`macro.event_blackout`）。
  全部參數進 `/engtest opt entry` 網格，讓 holdout 決定，不憑感覺調。
- **規則**：改進場邏輯前先拉反思帳本與 journal 分桶（AGENT_PLAYBOOK 的診斷代理模板）；
  「勝率低」的第一個假設是**時點**而不是**指標**。

### D16. 寬表聯集對齊的尾端缺值 → 標籤 NaN → 閘門被 NaN 誤放行（#55、#58）

- **症狀**：夜間 `meta.json` 的 OOS `equal_sharpe`／`sized_sharpe` 為 `NaN`，`by_mech.time.avg_r` 為 `NaN`。
- **原因**：全宇宙日期聯集的寬表裡，個別代碼在某天沒有收盤；時間柵欄出場直接取 `close[last]` → `NaN`。
  閘門以「`sized <= equal` 就不通過」寫法判斷，NaN 比較恆為 False → 那一項**被跳過、閘門誤放行**（#58 實測）。
- **修法**：時間柵欄改取區間內最後有效收盤（尾段缺值標 `data_end`）；樣本層丟棄非有限標籤/特徵並計數（`n_dropped`）；
  `gate()` 對每個比較值先檢查 `math.isfinite`，非有限一律不通過。
- **規則**：寬表計算的任何「取第 k 根」都要假設可能是 NaN；**閘門一律「證明通過才放行」**，比較前確認輸入有限。

### D17. 加碼不要求追蹤已啟動 → 放寬追蹤啟動點時，硬停損打在放大後的部位（#57）

- **症狀**：`trail_activate_r` 調到 2.0、`pyramid_r` 維持 1.0 時，+1R 加碼後回落到硬停損，損失超出 `risk_pct` 預算。
- **原因**：加碼後 `entry`/`rps` 不變，硬停損 `entry − stop_mult×rps` 套在整個部位；預設兩者皆 1.0 才碰不到。
- **修法**：加碼條件加 `r_peak ≥ trail_activate_r`（追蹤已啟動 → 保本地板保護整個部位）。
  殘餘風險：加碼那段最多回到原始進場價（保本地板），約 0.5R。網格也不再掃 `trail_activate_r`。
