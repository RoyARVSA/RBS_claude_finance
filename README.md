...
- **候選篩選與治理（P5/P6）**：`/screen` 從選股池與 AI 主題層挑 watchlist 之外的候選（Stage 3 限額補品質與修正動能、品質否決剔除、綜合分排名；只建議不自動加入）；`/valreport` 每月治理月報——估值層準不準：各判定的事後命中率、公允價值穩定度、MoS 因子 IC（重疊修正）、覆蓋與過期、指引覆蓋
