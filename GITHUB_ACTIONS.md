...
> **進場品質層（2026-09 診斷後新增）**：追高濾網——價格高於 MA20 超過 `eng_entry_max_ext_atr`（預設 2）個 ATR
> 且近 5 日漲幅 > `eng_entry_max_ret5d`（6%）才算噴出、不開新倉「等回檔」（平穩趨勢不擋）、加碼上限 `eng_pyramid_max_ext_atr`（1.5）、加碼最低評分 `eng_pyramid_min_score`（0.5）、`/set eng_neutral_pyramid on|off`；
> 大盤 regime=neutral（含氣象台「偏多但廣度弱」，廣度 <40 否決 risk_on）新倉風險 ×`eng_neutral_risk_mult`（0.5）且不加碼；
> **反思節流** `adaptive_throttle_enabled`（預設開）：近 30 次看多判斷命中率 <40% → 新倉風險再 ×0.5、停加碼；
> **事件靜默窗** `event_blackout_enabled`（預設開）：FOMC 會期兩天（`event_blackout_days` 1＝決議前一天起）不開新倉、不加碼，出場照常。
> `/autotrade` 顯示三者狀態；全部參數可由 `/engtest opt entry` 走樣本外驗證。
...
| 估值 | `/model TICKER [set k=v…\|clear]` — 公司模型引擎：PIT 三表（yfinance 主、Finnhub as-reported 備援）→ 驅動推導（共識/指引覆蓋前兩年、3 年均值、線性淡出）→ 專業 WACC（Blume β、合成信評利差、市值權重）→ 5+5 年 FCFF、終值 ROIC 淡出（預設保留歷史溢價 50%、上限 +10pp、bear 價值中性；再投資率 = g/ROIC）→ 熊/基/牛 + 蒙地卡羅 + WACC×g 敏感度 + 反向 DCF（市價隱含成長）→ 十條審核（未過=待審）→ 品質評分（Piotroski/Altman/Beneish/Sloan/ROIC 價差）→ MoS 分層門檻判定；金融股自動走 RIM（剩餘收益）、地產走股利 H-model；`set` 覆蓋驅動（`opm_target=0.24 beta=1.4 rev_g=0.3,0.25,0.2 guidance_rev=14000`；RIM 用 `payout=0.6 roe_target=0.15`、DDM 用 `div_g=0.04`）、`clear` 還原；估值歷史加密存 state；`/model` 看清單。**只調部位不觸發進場（P3 才接引擎）**、`/dcf AAPL [成長%]` — 簡版 DCF 內在價值（FCF→WACC→期中折現→終值→隱含股價；可覆蓋成長率假設） |
