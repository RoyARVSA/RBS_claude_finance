...
| 模擬交易 | `/autotrade on\|off`、`/alpha`（資訊疊加層現況）、`/positions`、`/pnl`、`/journal [N]`、`/checkup`（行為體檢：追高/頻率/太早出場/持有期）、`/attrib`（機制歸因：各出場/進場機制損益/勝率/賣後追蹤）、`/shadow`（舊邏輯 vs 新引擎對照）、`/mirror [init 現金 代碼:股數:成本…\|reset]`（鏡像帳：引擎接管你的實倉起點自主模擬）、`/rebalance [hrp\|max_sharpe\|min_vol\|erc\|equal]`（持倉再平衡顧問）、`/closeall` |
