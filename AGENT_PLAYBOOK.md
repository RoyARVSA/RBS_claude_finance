...
3. **驗證品質靠自覺**：自己寫的碼自己看不出盲點。實績：本專案的子代理對抗驗證在
   ...
   接近 100%：沒有一批是乾淨的**。trade_engine 重製再抓 2H+3M（API 失敗誤判空倉→簿記
   全滅+超限重複買、零股 qty=0 賣單無限重試 45 分鐘內誤觸保險絲、保險絲到期用同批舊
   事件再武裝讓 3 天冷卻實際變 9 天）——**共通教訓：驗證代理要求「寫新場景試著打破它」
   比只讀碼有效，這批 High 全是子代理自己跑模擬跑出來的**。→ **對策**：§3 模板不可省略。
